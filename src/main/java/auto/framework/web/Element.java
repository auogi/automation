...
public class Element implements ISearchContext {
	...
	public WebElement toWebElement(){
		...
		SearchContext searchContext = WebManager.getDriver();
		//searchContext.until(ExpectedConditions.presenceOfElementLocated(By.id("")));
		if(this.parent!=null && this.parent instanceof Element){
			Element parentElement = (Element) this.parent;
			...
			try{
				  timeout = Integer.parseInt(TestManager.Preferences.getPreference("webdriver.objectsearch.timeout"));
				  // is an integer!
			...
			return (new WebDriverWait(WebManager.getDriver(), timeout)).until(ExpectedConditions.presenceOfElementLocated(by));
		}
	...
	*/
	...
	public Boolean copyAndPaste(String textValue){
		...
		if(prereq){
			Toolkit toolkit = Toolkit.getDefaultToolkit();
			  ...
			  clipboard.setContents(new StringSelection(textValue), null);		  
			  this.toWebElement().sendKeys(Keys.chord(Keys.CONTROL, "v"), "");			  
			  success = true;
		}
		
		ReportLog.logEvent(success, "[" + name + "] Type \"" + textValue + "\"");
...
//		}
	...
	protected void doClick(){		
		if(this.getTagName().equalsIgnoreCase("input")){					
			...
			try
			{
				if(toWebElement().isDisplayed()){
					toWebElement().click();	
				}else{
					ReportLog.failed("Element is not displayed");
				}
									
			   //WebDriverWait wait = new WebDriverWait(WebManager.getDriver(), 60);
			...
			{				
			  if(e.getMessage().contains("Element is not clickable")){
				  		try {
				  			ReportLog.addInfo("Element is not Clickable. Retry will be done after 10 Secs..");				  			
							Thread.sleep(10000);
	...
	public Boolean click(Boolean Report){
		...
		if(prereq) {
			try{
				...
				ReportLog.verifyTrue(false, "[" + name + "] Click element");				
				throw e;
			} catch(Exception e){
				throw e;
			}
			...
			if(detailLog){
				if(Report){
					ReportLog.verifyTrue(success, "[" + name + "] Click element");
				}
			}
		return success;
	}
...
//sendKeys
	...
	public Boolean sendKeys(Keys key){
		...
		ReportLog.verifyTrue(success, "[" + name + "] Type \"" + key + "\"");
		return success;
	}
	
	public Boolean typeInField(String value){
		Boolean prereq = isInteractive(); 
		Boolean success = false;
		if(prereq) { 
	    String val = value; 
		...
		ReportLog.verifyTrue(success, "[" + name + "] Type \"" + value + "\"");
		return success;
	}
	...
	public Boolean submit() {
		...
		ReportLog.verifyTrue(success, "[" + name + "] Submit");
		return success;
	}
...
//clear
	public void clear() {
		ReportLog.verifyTrue(true, "[" + name + "] Clear");
		toWebElement().clear();
	}
...
//enabled
	public Boolean isEnabled() {
		try {
			Boolean isEnabled = toWebElement().isEnabled(); 				
			return isEnabled;
