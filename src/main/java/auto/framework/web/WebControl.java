...
public class WebControl {
	...
	public static void checkAlert() {
        try {
            WebDriverWait wait = new WebDriverWait(WebManager.getDriver(), 2);
            wait.until(ExpectedConditions.alertIsPresent());
            Alert alert = WebManager.getDriver().switchTo().alert();
            alert.accept();
        } catch (Exception e) {
            //exception handling
        }
...
/*         	String base64Encoded = "";
 			Screenshot elementScreenShot = new AShot().shootingStrategy(ShootingStrategies.viewportPasting(100)).takeScreenshot(WebManager.getDriver());
         	//Screenshot elementScreenShot = new AShot().shootingStrategy(new ViewportPastingStrategy(1000)).takeScreenshot(WebManager.getDriver());//
         	ByteArrayOutputStream baos = new ByteArrayOutputStream();
             ImageIO.write(elementScreenShot.getImage(), "png", baos);
             baos.flush();
...
*/         	
	...
	public static void takeScreenshot(String name, String dir) {
        ...
        try {
        	...
        	String timestamp = dateFormatTimeStamp.format(new Date());
        	//String filename = timestamp+ FilenameUtils.getName(screenshot.getCanonicalPath());
        	String filename = name; //   timestamp+ FilenameUtils.getName(screenshot.getCanonicalPath());
        	File tmpDir = new File("./src/test/resources/screenshots");//FileUtils.getTempDirectory();
			//File sessionDir = new File(tmpDir, ((RemoteWebDriver) driver).getSessionId().toString() );
        	File sessionDir = new File(tmpDir, dir);
        	if(sessionDir.exists() || sessionDir.mkdir()){
				tmpDir = sessionDir;
			}
			File tmpFile = new File(tmpDir,filename);
        	...
        	ReportLog.attachFile(name, tmpFile.toURI().toURL().toString());
	...
	private static java.net.Proxy getNetProxy(){
		Proxy proxy = (Proxy) WebManager.getRemoteDriver().getCapabilities().getCapability(CapabilityType.PROXY);
		java.net.Proxy javaProxy = null;
		if(proxy==null){
			proxy = WebManager.getProxySettings();
		}
		if(proxy!=null){
			...
			case MANUAL:
				...
				javaProxy = new java.net.Proxy(java.net.Proxy.Type.HTTP, new InetSocketAddress(proxyAddress[0], Integer.valueOf(proxyAddress[1].trim())));
				break;
			case PAC:
				break;
			case RESERVED_1:
...
//	}
	...
	public static Window openScreenshot(String url) {
		...
		try {
			...
			try {
					...
					absoluteURL = new URL(WebManager.getDriver().getCurrentUrl());
					Window win = open(absoluteURL,url);
					takeScreenshot();
					return win;
						}	
				Window win = open(null,url);
				takeScreenshot();
				return win;
			} catch (MalformedURLException e) {
				return open(null,url);
	...
	public static Window openScreenshot(String url,String name, String directory) {
	...
	public static void screenshotBaseline(String name, String directory) {		
    	Screenshot fpScreenshot = new AShot().shootingStrategy(ShootingStrategies.viewportPasting(1000)).takeScreenshot(WebManager.getDriver());       	
        //ImageIO.write(fpScreenshot.getImage(),"PNG",new File("D:///FullPageScreenshot.png"));        
    	//File tmpDir = new File("./src/test/resources/screenshots/baseline");
    	
    	File tmpDir = new File("C://screenshots");    	
    	File sessionDir = new File(tmpDir, directory);
    	
    	if(!sessionDir.exists()){
    		sessionDir.mkdir();	
    	}
    	    	
		File tmpFile = new File(sessionDir,name+".png");
		
		try {
			ImageIO.write(fpScreenshot.getImage(),"PNG",tmpFile);
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	public static void screenshotCompare(String name, String directory) {		
    	    	        
    	//File baselineDir = new File("./src/test/resources/screenshots/baseline");
		File baselineDir = new File("C://screenshots");
    	File sessionDir = new File(baselineDir, directory);
    	BufferedImage expectedImage = null;
    	Screenshot scrnshot = new AShot().shootingStrategy(ShootingStrategies.viewportPasting(1000)).takeScreenshot(WebManager.getDriver());    
    	BufferedImage actualImage = scrnshot.getImage();
    	String base64DiffEncoded="";
    	String base64ActEncoded="";
    	String base64ExpEncoded="";
		...
		File tmpFile = new File(sessionDir,name+".png");
		//File tmpDiffFile = new File(sessionDir,name+"diff.png");
		
		try {
			expectedImage = ImageIO.read(tmpFile);
        ...
        if(diff.hasDiff()){
        	ReportLog.warning("Base Image is the same the screenshot");
        	ByteArrayOutputStream diffOutput = new ByteArrayOutputStream();
        	ByteArrayOutputStream actualOutput = new ByteArrayOutputStream();
        	ByteArrayOutputStream expectedOutput  = new ByteArrayOutputStream();
        	try {
        		ImageIO.write(diff.getMarkedImage(), "png", diffOutput);			
        		ImageIO.write(actualImage, "png", actualOutput);
        		ImageIO.write(expectedImage, "png", expectedOutput);
        		diffOutput.flush();
        		actualOutput.flush();
        		expectedOutput.flush();
				byte[] encodeDifBase64 = Base64.encodeBase64(diffOutput.toByteArray());
				byte[] encodeActBase64 = Base64.encodeBase64(actualOutput.toByteArray());
				byte[] encodeExpBase64 = Base64.encodeBase64(expectedOutput.toByteArray());
				base64DiffEncoded = new String(encodeDifBase64);
				base64ActEncoded = new String(encodeActBase64);
				base64ExpEncoded = new String(encodeExpBase64);
				diffOutput.close();
				actualOutput.close();
        	...
        	ReportLog.attachScreenshot("Merged Image", base64DiffEncoded);
        	ReportLog.attachScreenshot("Expected Image", base64ExpEncoded);
        	ReportLog.attachScreenshot("Actual Image", base64ActEncoded);
        }else{
        	ReportLog.passed("Base Image is the same the screenshot");
        }
        
        /* 
        try {
			ImageIO.write(diffImage,"PNG",tmpDiffFile);
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		*/
        //ReportLog.assertTrue(diff.equals(diff), "Images are Same");
