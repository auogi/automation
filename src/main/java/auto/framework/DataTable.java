...
class DataHolder {
	...
	protected Recordset executeQuery(String sheet, String... where) {
		...
		try {
			...
			String dataTable = "./src/test/resources/data/"; 
					
			/*if( System.getProperties().containsKey("data")){
				dataTable = System.getProperty("data");
			}
			
			if( System.getProperties().containsKey("env.data")){
				dataTable = System.getProperty("env.data");
			}*/
			
			String defaultDt = "DataTable.xls";
			String customDt = TestManager.Preferences.getPreference("data");
			
			if(customDt!=null){
				if(new File("./src/test/resources/data/"+customDt).exists()){
					dataTable = dataTable+customDt;
				} else {
					dataTable = dataTable+defaultDt;
				}
			}
